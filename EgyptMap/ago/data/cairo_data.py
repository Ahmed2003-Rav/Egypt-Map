class CairoData:
    ...
    def _load_transport(self):
        ...
        self.metro_lines = [
            {"id": "M1", "name": "Line 1 (Helwan-New Marg)", "stations": [12,1,3,"F2",11], "passengers": 1500000, "distance": 25.5},
            {"id": "M2", "name": "Line 2 (Shubra-Giza)", "stations": [11,"F2",3,10,8], "passengers": 1200000, "distance": 18.7},
            {"id": "M3", "name": "Line 3 (Airport-Imbaba)", "stations": ["F1",5,2,3,9], "passengers": 800000, "distance": 22.3}
        ]
    ...
    def get_road_between(self, from_id, to_id):
        ...
        try:
            ...
            return None
    def get_metro_line(self, line_id):
        """Get metro line by ID"""
        try:
            return next((line for line in self.metro_lines if line['id'] == line_id), None)
        except Exception as e:
            print(f"Error getting metro line {line_id}: {e}")
            return None

    def get_bus_route(self, route_id):
        """Get bus route by ID"""
        try:
            return next((route for route in self.bus_routes if route['id'] == route_id), None)
        except Exception as e:
            print(f"Error getting bus route {route_id}: {e}")
            return None
